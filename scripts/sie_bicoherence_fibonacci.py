#!/usr/bin/env python3
"""
B37 — Targeted bicoherence + Fibonacci amplitude-product coupling.

Tests explicit Fibonacci-additive coupling at five specific bifrequency pairs
(f_a, f_b) → f_c = f_a + f_b:

  1. (7.82, 7.82) → 15.64        self-doubling of SR1
  2. (4.84, 7.82) → 12.66        φ⁻²·SR1 + SR1 → φ·SR1  (Fibonacci step)
  3. (7.82, 12.67) → 20.49       SR1 + φ·SR1 → φ²·SR1   (Fibonacci step, near SR3)
  4. (7.82, 19.95) → 27.77       SR1 + SR3
  5. (12.67, 19.95) → 32.62      φ·SR1 + SR3 (near β-γ boundary)

Two coupling measures per bifreq per event window:

  Phase-triad PLV (bicoherence-like):
    B = |mean( exp(i·(φ_a + φ_b − φ_c)) )|
    Ranges 0..1. Event-vs-control elevation indicates phase-phase nonlinear
    coupling at f_a + f_b.

  Amplitude-product correlation:
    r = Pearson corr between [|A_a(t)| · |A_b(t)|]  and  |A_c(t)|  over window
    If Fibonacci-additive, the product of lower-frequency envelopes should
    predict the higher-frequency envelope.

Event windows: 6 s centered at t0_net + 1 s. Controls: matched non-event
windows ≥30 s from any event.
"""
...
BAND_HW = 0.6  # half-width for narrowband filter (1.2 Hz bandwidth)
WIN_SEC = 6.0
EVENT_LAG = 1.0
...
def triad_plv(phase_a, phase_b, phase_c):
    """Phase-triad PLV: |<exp(i·(phi_a + phi_b - phi_c))>|.
    Insensitive to length n for n > ~100."""
    d = phase_a + phase_b - phase_c
    return float(np.abs(np.mean(np.exp(1j * d))))


def score_window(y_win, fs):
    """Compute (triad_plv, amp_prod_corr) for each of the 5 bifreqs."""
...
def process_subject(args):
    ...
    for t0 in t_events:
        ...
        if s is not None: ev_mats.append(s)
    ct_mats = []
    for tc in t_controls:
        s = run_window(tc)
        if s is not None: ct_mats.append(s)
    if not ev_mats or not ct_mats:
        return None
    ev = np.nanmean(np.array(ev_mats), axis=0)   # (n_bif, 2)
...
def main():
    ...
    print(f"Subjects: {len(tasks)}  bifreqs: {len(BIFREQS)}")
    ...
    print(f"\n=== Phase-triad PLV (bicoherence proxy): event vs control ===")
    ...
    print(f"\n=== Amplitude-product correlation |A_a·A_b| vs |A_c|: event vs control ===")
    print(f"{'bifreq':<26} {'event':>10} {'control':>10} {'Δ':>10} {'p':>10}")
    for k, (name, fa, fb, fc) in enumerate(BIFREQS):
    ...
    plt.suptitle(f'B37 — Targeted bicoherence + Fibonacci amplitude coupling '
