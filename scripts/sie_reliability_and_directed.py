#!/usr/bin/env python3
"""
B44 — Within-subject reliability of network-identity patterns + directed
coupling between SR1, β16, SR3 networks.

Test 1 (reliability): For each band, correlate each subject's Q4 topography
against the leave-one-out group Q4 mean; similarly for Q1. Also compute
within-subject Q4 × Q1 correlation. Tests whether the Figure 3 network-
identity finding holds at individual-subject level.

Test 2 (directed coupling): For each event window, extract narrowband
envelopes for SR1 (7-8.2 Hz), β16 (14.5-17.5), SR3 (19.5-20.4). Compute
envelope cross-correlation with lag search to identify which network leads.
Compare event vs control windows.

Uses the B43 topography CSV (already computed) for Test 1; recomputes per-
event envelope traces for Test 2.
"""
...
TOPO_CSV = os.path.join(OUT_DIR, 'topography_q4_vs_q1.csv')
os.makedirs(OUT_DIR, exist_ok=True)
warnings.filterwarnings('ignore')
import mne
mne.set_log_level('ERROR')

# Directed coupling: envelope bands
SR1_BAND = (7.0, 8.3)
...
def envelope_xcorr_lag(env_a, env_b, fs, lags_s=LAGS_S):
    """Cross-correlation across lags. Positive lag = env_b lags env_a."""
...
def process_subject_directed(args):
    ...
    def run_window(t_center):
        ...
        # Cross-correlations: pair → (lag_of_peak, r_at_peak, r_at_zero_lag)
        def xc(a, b):
...
def reliability_analysis():
    """Test 1: topographic reliability per subject using B43 CSV."""
    print("\n" + "=" * 64)
    print("Test 1: Topographic reliability of network identity")
    print("=" * 64)
    df = pd.read_csv(TOPO_CSV)
    # shape: one row per subject × band × channel, but the B43 CSV already
    # collapsed to grand averages. We need per-subject data.
    # Since B43 saved only grand means, compute a proxy from re-loading
    # the result — but data not saved per-subject. Use per-subject topos
    # by re-computing from the topography CSVs — same data available here.
    # Actually topography_q4_vs_q1.csv has: quartile, band, channel, ratio
    # (grand-average only). For per-subject, would need re-run.
    print("  topography_q4_vs_q1.csv contains grand-average values only.")
    print("  Per-subject topos not saved — would need re-run.")
    print("  Skipping Test 1 in this script (see B45 follow-up if needed).")
    return None


def main():
    # Run directed coupling analysis
    ...
    with Pool(n_workers) as pool:
        results = pool.map(process_subject_directed, tasks)
    ...
    # Test 2: directed coupling aggregate
    print(f"\n" + "=" * 64)
    print("Test 2: Envelope cross-correlation lag, event vs control")
    print("=" * 64)
    ...
    # Per-subject lag distribution for SR1 → β16 (the key directionality test)
    ...
    plt.suptitle(f'B44 — Envelope directed coupling (LEMON Q4, n = {len(results)})',
                 y=1.02, fontsize=12)
    plt.tight_layout()
