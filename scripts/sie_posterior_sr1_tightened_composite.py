#!/usr/bin/env python3
"""
B46 re-run on composite v2 detector.

Posterior-restricted SR1 test. Uses posterior channels (O/PO/P/TP + T7/T8)
to strengthen the surviving B45 reliable SR1 posterior finding.

Three outputs on posterior-mean signal:
  Test 1: IAF × SR1 peak scatter (posterior-mean, mirror of B20 Fig 1).
  Test 2: Subject-to-group SR1 topographic reliability (posterior only).
  Test 3: SR1 amplitude × IAF scatter.

Envelope B46: IAF 9.74 Hz vs Q4 SR1 peak 7.79 Hz; ρ = −0.115, slope −0.025.
IAF-independence strengthened where the SR1 signal actually lives.

Cohort-parameterized.

Usage:
    python scripts/sie_posterior_sr1_tightened_composite.py --cohort lemon
"""
...
def main():
    ...
    print(f"\n{'='*64}")
    print(f"{args.cohort} composite · Test 1: Posterior IAF × SR1 peak")
    print(f"{'='*64}")
    ...
    print(f"  Envelope B46: ρ = −0.115, slope = −0.025")

    print(f"\n{'='*64}")
    print(f"{args.cohort} composite · Test 2: Posterior-only SR1 topographic reliability")
    print(f"{'='*64}")
    ch_counts = {}
    for r in results:
    ...
    print(f"  Envelope B46: median ρ = 0.52, pct > 0.5 = 55%  (all-channel)")
    ...
    plt.suptitle(f'B46 composite — Posterior-restricted SR1 tightened '
