#!/usr/bin/env python3
"""
B13 re-run on composite v2 detector.

Identical analysis to scripts/sie_sr_band_1f_normalized.py (the envelope cohort
run). Reads events from exports_sie/<cohort>_composite/ and template_rho from
outputs/schumann/images/quality/per_event_quality_<cohort>_composite.csv.

Per subject and event:
  1. Sliding Welch (4-s window, 1-s hop) on the mean-channel signal over 2-20 Hz
  2. Per window: fit log-log linear aperiodic on [2,5] U [9,20] Hz (exclude 5-9 Hz)
  3. SR-band [7.0, 8.2] Hz peak power per window; narrowband excess =
     log10(peak_power) − log10(aperiodic_at_peak_freq)
  4. Event excess-over-baseline = event_excess − all-time median excess (log10)

Outputs to outputs/schumann/images/psd_timelapse/<cohort>_composite/.

Usage:
    python scripts/sie_sr_band_1f_normalized_composite.py --cohort lemon
    python scripts/sie_sr_band_1f_normalized_composite.py --cohort lemon_EO
"""
...
def main():
    ...
    print(f"\n=== {args.cohort} composite — per-subject 1/f-normalized boost ===")
    ...
    if 'rho_q' in ev.columns and ev['rho_q'].notna().any():
        print(f"\n=== {args.cohort} composite — 1/f-normalized by template_rho quartile ===")
    ...
    ax.set_title(f'{args.cohort} composite · 1/f-normalized (n={len(sub)})')
    ...
    plt.suptitle(f'B13 · SR-band event-boost · 1/f-aperiodic normalized · {args.cohort} composite v2',
                 y=1.02, fontsize=12)
    plt.tight_layout()
